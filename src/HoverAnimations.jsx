...
export default function HoverAnimations({ Hitboxes, Meshes }) {
  ...
  const socialIcons = [
    { name: 'github', hitbox: Hitboxes.githubHitbox, mesh: Meshes.githubMeshRef },
    { name: 'linkedIn', hitbox: Hitboxes.linkedInHitbox, mesh: Meshes.linkedInMeshRef },
    { name: 'email', hitbox: Hitboxes.emailHitbox, mesh: Meshes.emailMeshRef },
  ]

  // Section buttons that only grow bigger when hovered
  const sections = [
  ...
  // Make social icons bigger AND float up
  ...
  return (
    <>
      {/* Social Icons - hover to make bigger AND float up */}
      {socialIcons.map(({ name, hitbox, mesh }) => (
